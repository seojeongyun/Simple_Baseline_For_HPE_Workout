...
def main(rank):
    ...
    valid_dataset = JointsDataset(cfg=config,
                         ...
                         task='validation' if config.TASK == 'train' or config.TASK == 'hard_exercise_finetune' else config.TASK,
                         transform=transforms.Compose([transforms.ToTensor(), normalize]))
    if config.USE_DDP:
    ...
    best_perf = 0.0
    if config.TASK == 'train' or config.TASK == 'hard_exercise_finetune':
        for epoch in range(config.TRAIN.BEGIN_EPOCH, config.TRAIN.END_EPOCH):
            ...
            if best_model:
                ...
                if config.MODEL.PRETRAINED:
                    torch.save(model.state_dict(),os.path.join(final_output_dir, '[Weight, New_Data, hard_ex_finetune] finetune_512_128.pth.tar'))
        ...
        final_model_state_file = os.path.join('/storage/jysuh/fitness_weights/',
                                              '[New_Data, finetune, hard_ex_finetune] final_state.pth.tar')
...
if __name__ == '__main__':
    ...
    setproctitle('HPE: hard_ex_finetune')
